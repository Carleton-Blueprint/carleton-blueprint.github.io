...
function Value({ data }: { data: ValueDataType }) {
  return (
    ...
    >
      <div className={`font-bold text-blueprint text-2xl md:text-4xl mb-3 ${data.flip && 'md:text-end'}`}>{data.title}</div>
      <div className={`${!data.flip && 'md:text-end'} text-justify md:text-start text-md md:text-lg`}>
        {data.description}
      </div>

      <div className={cn("hidden md:flex top-1/2 w-[100px]  md:w-[180px] lg:w-[220px] aspect-square transform absolute -translate-y-1/2  justify-center items-center z-4 right-2 translate-x-full", data.flip && "-left-2 -translate-x-full")}
      >
...
export default function OurValuesBlock() {
  return (
    <BlockContainer title='Our Values' flip inner roundedCorners margin='bottom'>
