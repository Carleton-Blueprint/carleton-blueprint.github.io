...
function Value({ data }: { data: ValueDataType }) {
  return (
    ...
    >
      <div className={`font-bold text-blueprint text-2xl md:text-4xl ${data.flip && 'md:text-end'}`}>{data.title}</div>
      <div className={`${!data.flip && 'md:text-end'} text-justify md:text-start text-md md:text-lg`}>
        {data.description}
      </div>
      <div
        className={`absolute ${
          data.flip ? '-left-2 -translate-x-full' : 'right-2 translate-x-full'
        } top-1/2 w-[100px] h-[100px] md:w-[250px] md:h-[250px] lg:w-auto lg:h-auto transform -translate-y-1/2 flex justify-center items-center`}
      >
